...
class Blockchain:
    ...
    def verify_transaction_signature(self, sender_public_key, signature, transaction):
        ...
        try:
            public_key.verify(bytes.fromhex(signature), bytes.fromhex(
                h.hexdigest()), ec.ECDSA(hashes.SHA256()))
            return True
        except ValueError:
            return False
    ...
    def submit_transaction(self, sender_public_key, recipient_public_key, signature, message):
